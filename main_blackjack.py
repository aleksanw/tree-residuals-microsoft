...
def run():
    ...
    for learning_iteration in range(1000):
        policy = Policy_EpsilonGreedy(q, epsilon=0.4)
        ...
        if learning_iteration % 1 == 0:
            ...
            reward_sum = avg(test_policy(greedy_policy, env) for _ in range(100))
            print(f"Episode {learning_iteration*batch_size} Reward {reward_sum} lr {learning_rate}")
