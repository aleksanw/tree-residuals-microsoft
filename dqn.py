...
def run(env_id):
    ...
    envs = {
        ...
        9:('CorridorBig-v5', CorridorEnv),
        10:('CorridorBig-v10', CorridorEnv),
        11:('CorridorFLNonSkid-v1', CorridorEnv)
    }
    ...
    with tf.Session() as sess:
        ...
        for epi in range(1, num_episodes+1):
            ...
            while (j < max_epLength) and (not d): #If the agent takes longer than 200 moves to reach either of the blocks, end the trial.
                ...
                if np.random.rand(1) < e or total_steps < pre_train_steps:
                   a = np.random.randint(0,env.env.action_space.n)
                else:
                    #if e == 0: print(a)
        ...
        _succ_epi, _acc_rew = evaluate(env2, sess, mainQN.predict, mainQN.input, visualize=False, v_func=mainQN.Value)
        print("Evaluation success rate over 100 episodes: {}%; Total reward: {} ".format(_succ_epi, _acc_rew))
